...
<div id="wrapper">
	...
	<div id="page-wrapper"
		 style="margin:0rem;background: linear-gradient(to bottom, #0052D4, #4364F7, #6FB1FC);">
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header" align="center" style="font-size: 60px;color: #FFFFFF;border: none;">
					Customer List</h1>
			</div>
			<!-- /.col-lg-12 -->
		</div>
			...
			<div class="panel-body">
				<form class="form-inline" method="get"
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="customerName" value="${custName }"
							   name="custName" />
					</div>
					<div class="form-group">
						...
						<select class="form-control" id="customerFrom" name="custSource">
							...
							<c:forEach items="${fromType}" var="item">
								<option value="${item.dict_id}" <c:if test="${item.dict_id == custSource}">
									selected</c:if>>
										${item.dict_item_name }
								</option>
							</c:forEach>
					...
					<div class="form-group">
						...
						<select class="form-control" id="custIndustry" name="custIndustry">
							...
							<c:forEach items="${industryType}" var="item">
								<option value="${item.dict_id}" <c:if test="${item.dict_id == custIndustry}">
									selected</c:if>>
										${item.dict_item_name }
								</option>
							</c:forEach>
					...
					<div class="form-group">
						...
						<select class="form-control" id="custLevel" name="custLevel">
							...
							<c:forEach items="${levelType}" var="item">
								<option value="${item.dict_id}" <c:if test="${item.dict_id == custLevel}">
									selected</c:if>>
										${item.dict_item_name }
								</option>
							</c:forEach>
					...
					<button type="submit" class="btn btn-primary"
							style="background-color:#f08c00;border: none;">查询</button>
				</form>
			</div>
		</div>
		<a href="#" class="btn btn-primary" data-toggle="modal" data-target="#newCustomerDialog"
		   onclick="clearCustomer()" style="background-color:#f08c00;border: none;">新建</a>
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<table class="table table-bordered table-striped">
						...
						<c:forEach items="${page.rows}" var="row">
							<tr align="center" style="border: none;bo">
								<td>${row.cust_id}</td>
								<td>${row.cust_name}</td>
								...
								<td>
									<div align="center"><a href="#" class="btn btn-primary btn-xs"
														   data-toggle="modal" data-target="#customerEditDialog"
														   onclick="editCustomer(${row.cust_id})"
														   style="font-size: 20px;">修改♻</a></div>
								</td>
								<td>
									<div align="center"><a href="#" class="btn btn-danger btn-xs"
														   onclick="deleteCustomer(${row.cust_id})"
														   style="font-size: 20px;">删除❌</a></div>
								</td>
							</tr>
						</c:forEach>
						</tbody>
...
<!-- 创建客户模态框 -->
<div class="modal fade" id="newCustomerDialog" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" style="
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<select class="form-control" id="new_custLevel" name="cust_level">
								...
								<c:forEach items="${levelType}" var="item">
									<option value="${item.dict_id}" <c:if test="${item.dict_id == custLevel}">
										selected</c:if>>${item.dict_item_name }</option>
								</c:forEach>
							</select>
						</div>
					</div>
